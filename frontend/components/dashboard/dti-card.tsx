            ...
            <>
              ...
              <div className="space-y-2.5">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-slate-500">Средний доход</span>
                  <span className="text-sm font-medium text-slate-900">
                    {formatMoney(health.dti_income)} / мес
                  </span>
                </div>

                <div className="mt-2 flex items-center justify-between">
                  <span className="text-sm text-slate-500">Сумма платежей</span>
                  <span className="text-sm font-medium text-rose-600">
                    {formatMoney(health.dti_total_payments)} / мес
                  </span>
                </div>
              </div>
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                {[
                  { label: '<30% - норма', bg: 'bg-emerald-100', text: 'text-emerald-700' },
                  { label: '30-40% - допустимо', bg: 'bg-amber-100', text: 'text-amber-700' },
                  { label: '>40% - опасно', bg: 'bg-rose-100', text: 'text-rose-700' },
                  { label: '>60% - критично', bg: 'bg-red-200', text: 'text-red-800' },
                ].map((zone) => (
                  <span
                    key={zone.label}
                    className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${zone.bg} ${zone.text}`}
                  >
                    {zone.label}
                  </span>
                ))}
