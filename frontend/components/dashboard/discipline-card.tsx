  ...
  return (
    <div className="mt-3">
      <svg viewBox="0 0 120 70" className="mx-auto h-[72px] w-full max-w-[180px]">
        ...
        />
        <text x="60" y="56" textAnchor="middle" className="fill-slate-800 text-[16px] font-semibold">
          {value.toFixed(1)}%
        </text>
      </svg>
...
export function DisciplineCard({
  ...
  const tip = worstViolation
    ...
    : (health.discipline ?? 0) < 60
      ? 'Выставь лимиты по основным категориям расходов'
    ...
    >
      ...
      <div ref={cardRef}>
        ...
        >
          ...
          {health.discipline === null ? (
            <p className="mt-4 text-xs text-slate-400">Выставьте лимиты в разделе Бюджет</p>
          ) : (
            <Gauge value={health.discipline} color={gaugeColor} />
          )}
          ...
          {isExpanded ? (
            <>
              ...
              <div className="space-y-2.5">
                ...
                {health.discipline_violations.length === 0 ? (
                  <p className="text-xs text-slate-400">Нарушений не обнаружено</p>
                ) : (
                  health.discipline_violations.slice(0, 3).map((violation) => (
                    <div key={violation.category_name} className="flex items-center justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-slate-400">
                          {violation.months_count} мес. подряд
                        </p>
                      </div>
                      <span className="shrink-0 rounded-full bg-rose-100 px-2.5 py-0.5 text-xs font-medium text-rose-700">
                        +{violation.overage_percent.toFixed(0)}%
                      </span>
                    </div>
                  ))
                )}
              ...
              <div className="mt-3">
                ...
                {health.discipline_history && health.discipline_history.length > 0 ? (
                  <div className="flex h-10 items-end gap-1.5">
                    {health.discipline_history.slice(-3).map((item, index, array) => {
                      ...
                      return (
                        <div key={item.month} className="flex flex-1 flex-col items-center gap-1">
                          ...
                          <div
                            ...
                            style={{
                              height: `${Math.max((item.value / 100) * 32, 4)}px`,
                              background: isLast ? barColor : `${barColor}80`,
                            }}
                          />
                          <span className="text-[9px] text-slate-300">{item.month}</span>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-xs text-slate-400">Накапливается история...</p>
                  </div>
                )}
              </div>
