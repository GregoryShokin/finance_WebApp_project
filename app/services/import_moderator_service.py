...
Твоя задача — выбрать:
1. operation_type — один из: "regular", "transfer", "credit_payment", "investment_buy", "investment_sell", "credit_disbursement".
2. direction — "income" или "expense" (даже если во входе "unknown").
3. predicted_category_id — ID категории из списка пользователя, или null если это transfer/investment/credit (тогда категория не нужна).
4. confidence ∈ [0, 1]:
...
class ClusterHypothesis(BaseModel):
    ...
    operation_type: str = Field(description="regular/transfer/credit_payment/investment_buy/investment_sell/credit_disbursement")
