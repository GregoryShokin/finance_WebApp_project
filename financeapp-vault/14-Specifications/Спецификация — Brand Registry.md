...
**История версий:**
- **v1.26 (2026-05-09) — Brand vs personal identifier: негативное правило.** Введён предикат `is_personal_identifier_row(skeleton, tokens)` и три точки его применения (§17). Строки, идентифицированные только phone/contract/person_name без org или sbp_merchant_id, перестают получать brand-match от резолвера (text/alias_exact blinded), не учат новых text-паттернов, и не вызывают эмиссию BrandCluster в import_cluster_service. Explicit user-bind через picker по-прежнему разрешён: fingerprint-binding сохраняется, auto-learn (только) пропускается. Это мост к задаче 5 (personal-counterparty entity), но уже сейчас предотвращает смешивание категорий «звонок маме» / «долг другу» / «аренда Брату» под одним брендом.
...
- **v1.25 (2026-05-08) — Phase C merge.** `Counterparty` как отдельная сущность абоlished. Brand становится единственным источником мерчант-личности. `transactions.counterparty_id`, `counterparty_fingerprints`, `counterparty_identifiers`, таблица `counterparties` — DROP (миграции 0067 + 0068). На их место: `transactions.brand_id`, `brand_fingerprints`, `brand_identifiers`. Per-user ререйминг переехал в новую таблицу `user_brand_display_names` (см. §2.5). DebtPartner остаётся отдельной сущностью — он никогда не был частью этого слияния. См. §16 «История миграции Counterparty → Brand».
...
Brand Registry — **единственная** мерчант-сущность в пайплайне (с v1.25). Её задача — дать стабильное человекочитаемое имя мерчанта поверх сырого описания банковской выписки и привязать это имя к транзакции для аналитики и категоризации.
...
- Подтверждение бренда на ОДНОЙ строке стампит `nd.brand_id`, биндит fingerprint в `brand_fingerprints`, **обучает новый паттерн** и подметает siblings во всей сессии одним проходом.
...
- Per-user display-label override (`UserBrandDisplayName`) даёт юзеру переименовать глобальный бренд для своих лент без форка самой brand-сущности (см. §2.5).
...
- Не пересекается с `DebtPartner`. Долги (operation_type='debt') — отдельная сущность, см. CLAUDE.md «Brand vs DebtPartner».
...
С v1.25 — шесть таблиц:

| Таблица | Миграция | Назначение |
|---|---|---|
| `brands` | `0064` | каноничная личность мерчанта |
| `brand_patterns` | `0064` | способы появления бренда в сыром тексте |
| `user_brand_category_overrides` | `0065` | per-user override категории-по-умолчанию для бренда |
| `brand_fingerprints` | `0067` | fingerprint → brand binding (заместил `counterparty_fingerprints`) |
| `brand_identifiers` | `0067` | identifier (phone/contract/iban/card) → brand binding (заместил `counterparty_identifiers`) |
| `user_brand_display_names` | `0067` | per-user override visible label, см. §2.5 |

Дополнительная FK: `transactions.brand_id` (миграция `0067`, NULL allowed для transfer/debt/credit). Заместил `transactions.counterparty_id` (DROP в миграции `0068`).
...
### 2.4 `BrandFingerprint` / `BrandIdentifier`

Заместили `counterparty_fingerprints` / `counterparty_identifiers` в миграции `0067`. Семантика идентична: пользовательский биндинг fingerprint'а или cross-account identifier'а на сущность мерчанта.
...
| `fingerprint` | str(32) | UNIQUE c `user_id` |
| `brand_id` | int FK | `brands.id ON DELETE CASCADE` |
| `confirms` | int | счётчик повторных привязок |

`BrandIdentifier` — то же, но keyed на `(identifier_kind, identifier_value)`. Поддерживаемые kinds: `phone`, `contract`, `iban`, `card`. Cross-account-резолв: один и тот же `+79991112233` в выписках двух разных счетов резолвится в один и тот же бренд.

### 2.5 `UserBrandDisplayName`

Per-user override visible-label'а бренда. Введён в Phase C step 1 (миграция `0067`) для случаев типа «глобальный бренд называется `Пятёрочка`, но я в своих лентах хочу видеть `Пятёрочка у дома`».
...
| `user_id` | int FK | `users.id ON DELETE CASCADE` |
| `brand_id` | int FK | `brands.id ON DELETE CASCADE` |
| `display_name` | str(128) | произвольный пользовательский ярлык |

UNIQUE `(user_id, brand_id)` — один override на пару. Не клонирует Brand, не форкает paterns — это чисто display-слой. Резолвер и автообучение работают на канонической личности; фронтенд показывает override-имя в чипах, заголовках карточек, истории. Удаление override → возвращается `Brand.canonical_name`.

`BrandConfirmService.confirm_brand_for_row` ставит `brand_display_name` в response поверх `brand_canonical_name`, чтобы клиент мог выбрать что рендерить.
...
## 14. Запрещённые text-токены (Forbidden text tokens)

_Добавлено 2026-05-09. Закрывает баг: «возврат покупки SBP сервиса Яндекса» резолвился в бренд "sbp" вместо "яндекс"._

### 14.1 Определение

**Запрещённый text-токен** — слово/аббревиатура, которая описывает платёжную инфраструктуру (rail, схему карты, код терминала), а не мерчанта. Такой токен **никогда** не должен становиться `kind='text'` паттерном бренда — ни через auto-learn, ни через ручной `add_pattern_to_brand`, ни через сидер.

### 14.2 Текущий список

| Токен | Объяснение |
|---|---|
| `sbp`, `сбп` | Система Быстрых Платежей — rail, не мерчант |
| `qsr`, `mop` | НСПК-коды категорий мерчанта (QuickServiceRestaurant, MerchantOther...). Числовой ID мерчанта внутри SBP-rail (`sbp_merchant_id`) — **легитимный** kind; слово "qsr"/"mop" — нет |
| `pos` | Point-of-sale terminal code |
| `atm` | ATM-код |
| `mir`, `visa`, `mastercard` | Карточные схемы |

### 14.3 Где применяется защита

1. **`_FILLER_TOKENS` в `brand_extractor_service.py`** — `extract_brand()` пропускает эти токены при обходе skeleton'а; они не возвращаются как brand-кандидат.

2. **`_FORBIDDEN_LEARN_TOKENS` в `brand_confirm_service.py`** — `_learn_pattern_from_row()` дополнительно проверяет кандидата перед upsert'ом. Guard нужен как second line of defence: если `extract_brand` в будущем изменится или токен поступит через другой путь — блок остаётся.

3. **`test_brand_seed_registry.py::test_no_seed_brand_has_sbp_as_text_pattern`** — CI-guard против случайного добавления запрещённых токенов в CSV.

### 14.4 Legit SBP-merchant-id НЕ затронут

`sbp_merchant_id` — это числовой ID мерчанта внутри SBP-rail (например `26033` для Вкусно-и-точки). Он извлекается через `SBP_EXPLICIT_RX` / `SBP_QR_SUFFIX_RX`, хранится в `ExtractedTokens.sbp_merchant_id` и матчится резолвером как `kind='sbp_merchant_id'`. Запрет касается только **литерала** "sbp"/"сбп" как text-паттерна.

Пример: `"26033 MOR SBP 0387"` → `sbp_merchant_id="26033"` → бренд "Вкусно и точка" (легитимно).  
Пример: `"возврат покупки SBP сервиса Яндекса"` → "sbp" попадает в filler → brand-токен "яндекс[а]" → бренд "Яндекс" (исправленное поведение).
...
- **`UserBrandDisplayName`** — per-user override visible-label'а бренда. Не клонирует Brand, не форкает паттерны (см. §2.5).
- **Fingerprint binding** — `BrandFingerprint(user_id, fingerprint, brand_id)`. Эффект на будущие импорты юзера. До v1.25 жил на `CounterpartyFingerprint`.
...
- **`clusters` view** — brand-binding / brand / fingerprint группировка. Opt-in для power-users.

---

## 17. Brand vs Personal Identifier — что НЕ становится брендом (v1.26)

### 17.1 Продуктовое правило

**Brand** — merchant с устойчивой категорией. «Пятёрочка» → Продукты всегда. «Яндекс Такси» → Такси всегда. Резолвер возвращает match → inline-prompt «Это Пятёрочка?» → одним кликом весь импорт разложен по категориям.

**Personal identifier** (phone, contract, person_name/ФИО) — это контрагент или связь, НЕ merchant. Один и тот же человек получает деньги на еду, долг, подарок, аренду, возврат — категория всегда разная. Автоматически полагать «это бренд» для personal-identifier-строки — ошибка домена.

Примеры:
- `«Перевод +79161234567»` (phone) — нельзя создавать brand «Мама» автоматически; завтра тот же телефон получит перевод за аренду.
- `«Погашение тела по договору КС123»` (contract) — нельзя создавать brand «Банк погашение»; следующий платёж — проценты, и там другая категория.
- `«Иванов Иван Иванович»` (person_name) — нельзя создавать brand «Иванов»; он получает деньги по разным поводам.

Если пользователь хочет явно назвать телефон/договор/человека брендом — это осознанный override. Он делается вручную через picker (fingerprint-binding сохраняется). Автоматика не предполагает.

### 17.2 Предикат `is_personal_identifier_row`

Реализован в `app/services/brand_extractor_service.py`:

```python
def is_personal_identifier_row(skeleton: str, tokens) -> bool:
    """True при наличии phone / contract / person_name без org / sbp_merchant_id."""
```

Принимает `ExtractedTokens` (dataclass) или `dict` из `normalized_data_json["tokens"]`.

| Условие | Результат |
|---|---|
| `tokens.phone is not None` AND `org is None` AND `sbp is None` | **True** |
| `tokens.contract is not None` AND `org is None` | **True** |
| `tokens.person_name is not None` AND `org is None` AND `sbp is None` | **True** |
| `tokens.counterparty_org is not None` (любой из выше) | **False** — org overrides |
| `tokens.sbp_merchant_id is not None` (любой из выше) | **False** — sbp overrides |
| Всё None | **False** |

### 17.3 Точки применения

**A — `BrandResolverService.resolve()`:**
После нахождения match по `kind ∈ {text, alias_exact}` — проверяет `is_personal_identifier_row(skeleton, tokens)`. Если True → возвращает `None` (не показывает «Это <brand>?»). Kind'ы `sbp_merchant_id` и `org_full` exempt — они сигнализируют реального мерчанта.

**B — `BrandConfirmService._learn_pattern_from_row()`:**
Если `is_personal_identifier_row(skeleton, tokens)=True` → silent skip, текстовый паттерн не создаётся. Fingerprint-binding (шаг перед вызовом learn) сохраняется — explicit пользовательская привязка не отменяется. Только авто-learn блокируется.

**C — `ImportClusterService._group_by_brand()`:**
Кластеры с `cluster.identifier_key ∈ {phone, contract, person_hash}` исключаются из BrandCluster-группировки. `BrandGroup` (явный binding через `BrandFingerprint` / `BrandIdentifier`) НЕ фильтруется — это осознанное user-действие.

### 17.4 Чего НЕ делает этот раздел

- Не создаёт новую сущность «personal counterparty» — это задача 5.
- Не убирает `DebtPartner` (отдельная сущность для долговых партнёров, без изменений).
- Не блокирует explicit manual override через picker — пользователь может привязать phone/contract к Brand сознательно.
- Не трогает `BrandGroup` (explicit binding level) — только `BrandCluster` (soft, auto-inferred level).
- Не изменяет логику §12.11 (card identifier — только для transfer-строк).

---

## 16. История миграции Counterparty → Brand (v1.25)

До 2026-05-08 в системе сосуществовали две merchant-сущности:
- `Counterparty` — пользовательский record с именем, биндился в `Transaction.counterparty_id`.
- `Brand` — recognition-слой, материализовывался в Counterparty при confirm'е.

Двойственность приводила к drift'у: разные UI-точки могли подтвердить одну и ту же row либо в Counterparty, либо в Brand, и они расходились. За один день dev-БД набирал ~40 desync-row'ов.

**Phase C** (5 шагов, 2026-05-07 → 2026-05-08) удалил Counterparty полностью:

| Шаг | Что сделано | Миграция |
|---|---|---|
| 1 | Создан `transactions.brand_id`, `brand_fingerprints`, `brand_identifiers`, `user_brand_display_names`. Data move: каждая `Counterparty` смапилась на существующий или новый `Brand` по case-fold имени; `transactions.counterparty_id` переписан в `brand_id`; биндинги перенесены. Pre-existing tables оставлены для отката. | `0067` |
| 2 | Backend сервисы переключены на двойную запись (и в CP, и в Brand) — на случай отката данных Step 1. | (no migration) |
| 3 | Frontend moderator переключён на Brand-native picker / chips / chronological. CP-fetch отрезан. | (frontend-only) |
| 4 | Backend write-paths закрыты со стороны CP. Read-paths (refund-history JOIN, cluster grouping) переключены на `Transaction.brand_id` / `BrandFingerprint`. Pre-flight тест: 0 row'ов с CP-stamp без brand-stamp. | (no migration) |
| 5 | DROP TABLE `counterparties`, `counterparty_fingerprints`, `counterparty_identifiers`. DROP COLUMN `transactions.counterparty_id`. Удалены: `app/models/counterparty.py`, `app/api/v1/counterparties.py`, `frontend/components/counterparties/`, и т.д. | `0068` |

**Что НЕ затронуто Phase C:**
- `DebtPartner` — отдельная сущность (см. CLAUDE.md, decision 2026-04-24). Phase C её не касался.
- `BrandPattern`, `UserBrandCategoryOverride` — структура и семантика без изменений.
- `Brand.is_global` дихотомия — глобальные seeded брендом сидером (`scripts/seed_brand_registry.py`), приватные user-created. Phase C сохраняет.

**Recovery path:** миграция `0068` имеет `downgrade()` → `NotImplementedError`. Один путь восстановления — `pg_dump` бэкап до запуска `alembic upgrade head` (на dev-DB снят 2026-05-08 как `backup_before_step5_*.sql`).
